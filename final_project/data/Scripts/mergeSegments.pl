...
sub execute
{
	...
	while (<$input>)
	{
		...
		$lineCounter++;
                ...
                $line =~ s/\"//g;
		my ($id, $chrom, $chr_start, $chr_stop, $num_mark, $seg_mean, $bstat, $p_value, $lcl, $ucl) = split(/\t/, $line);

		if($id eq "ID")
		{
			## Skip header ##
		}
        elsif(substr($chrom, 0, 2) eq "GL")
        {
            ## Skip GL...
        }
        elsif($ucl eq "NA")
...
{                               # replace with real execution logic.
	...
	while (<$input>)
	{
		...
		my ($chrom, $chr_start, $chr_stop, $arm_name) = split(/\t/, $line);
